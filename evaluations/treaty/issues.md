...
### §10 observation: `treaty exec` waits on an interactive TTY stdin
`exec` reads JSONL from stdin by design. If an agent runs it with an inherited TTY stdin and no pipe, it blocks until EOF. Not scored against §10 because stdin is the command's data channel, but `exec` could refuse when `stdin.isatty()` is true and no input is piped.
Discovered during §10 evaluation on 2026-09-24.
